...
- `protobuf<7` is now pinned transitively, which downgraded `grpcio-tools`
  ...
  (gencode 7.35.1 against runtime 6.33.6). That bit while the gRPC wire
  existed; the wire, its stubs and its tooling are gone now, but a
  protobuf major in either direction can bite any remaining consumer
  again.
