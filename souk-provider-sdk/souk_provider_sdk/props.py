...
class VerifiedActor(BaseModel):
    """One hop of a verified actor chain, resolved against souk's roster: the hop's signing key, and the agent name registered under it (None if unregistered)."""
...
class CallerProps(BaseModel):
    """souk's `forwardedProps.caller` entry, as this side validates it: the verified caller identity attached to a dispatched run.

    The independent twin of `souk.props.CallerProps` — neither package
    imports the other; the delivered-run frame in
    `docs/contract-vectors.json` pins the two byte-for-byte. Validate with
    this model rather than restating it: `chain` is None whenever souk
...
class KyokForwardedProps(BaseModel):
    ...
    The independent twin of `souk.kyok.KyokForwardedProps`, pinned by the
