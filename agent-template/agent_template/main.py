...
def make_run_stream(agent: Agent):
    async def run_stream(run_input: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
        ...
        deps = AgentDeps(progress_queue=combined, thread_id=run_input.get("thread_id"))

        async def drain_adapter() -> None:
            try:
                run_input_obj = AGUIAdapter.build_run_input(json.dumps(run_input).encode())
                adapter = AGUIAdapter(agent=agent, run_input=run_input_obj, deps=deps)
                async for event in adapter.run_stream():
                    await combined.put(event.model_dump(mode="json"))
            except Exception:
                logger.exception("agent run failed for run_id=%s", run_input.get("run_id"))
            finally:
