...
| **Ships** | `souk` (the network-free core library) + the two contract SDKs, [`souk-provider-sdk`](souk-provider-sdk/) and [`souk-caller-sdk`](souk-caller-sdk/), both transport-free | The reference gateway, the transport SDKs (`souk-agent-sdk`, `souk-client-sdk`) and the reference providers |
...
| [`souk-caller-sdk/`](souk-caller-sdk/) | **What a caller and souk agree on**, from the caller's side: the Keep Your Own Key bridge and its port. Zero dependencies — no transport *and* no LLM client, since choosing how you call your own model is the decision KYOK exists to leave you. See its [README](souk-caller-sdk/README.md) |
...
Four SDK names circulate, and they are two pairs. **The contract SDKs are here** and define the interactions: `souk-provider-sdk` and `souk-caller-sdk`. The transport SDKs are the sockets around them and live in [AgentSoukServer](https://github.com/hukaichun/AgentSoukServer): `souk-agent-sdk` (a client for the gateway's provider WebSocket) wraps the first, `souk-client-sdk` (the caller's side) the second — along with the reference providers (`agent-template`, `providers/*`) and the directory UI (`souk-directory`). In each pair the package here is named for the *role in the relationship* and the one there for the *thing you run*. The gateway repo owns both ends of every wire it defines, and the clients and examples live with the stack they front.
