...
def test_sdk_module_does_not_import_funduq(module: Path) -> None:
    ...
    assert not offenders, (
        ...
        "runs in a different process with no access to funduq's code; it may share only "
        "the wire contract, which each side computes independently so funduq's suite "
...
def test_funduq_is_not_installable_as_a_dependency() -> None:
    ...
    assert '"funduq' not in declared and "'funduq" not in declared, (
        "funduq-provider-sdk's dependencies include funduq, which puts core one import "
