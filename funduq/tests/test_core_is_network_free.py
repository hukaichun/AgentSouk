...
def test_core_module_imports_no_transport(module: Path) -> None:
    ...
    assert not offenders, (
        f"{module.name} imports {sorted(offenders)}, which makes funduq's core depend on a "
...
def test_no_transport_is_even_installable_as_a_dependency() -> None:
    ...
    assert not offenders, (
        ...
        "funduq-server repo; a transport listed here puts it one import away from core again."
    )
