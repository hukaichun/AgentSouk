...
class _Identity(ProviderIdentity):
    ...
    async def register(self, funduq: Funduq, *names: str):
...
async def test_a_registration_signature_is_not_a_deletion_order(funduq):
    identity = _Identity()
    await identity.register(funduq, "translator")
    timestamp = int(time.time())

    replayed = identity.registration_signature(["translator"], timestamp)

    with pytest.raises(InvalidRegistration):
        await funduq.delete_agent(
...
async def test_deleting_needs_the_key_that_registered(funduq):
...
async def test_a_stale_deletion_is_refused(funduq):
    identity = _Identity()
    await identity.register(funduq, "translator")
...
async def test_an_unused_agent_is_deleted_and_its_name_is_free_again(funduq):
    identity = _Identity()
    await identity.register(funduq, "typo")
    agent = AgentRef(provider_key=identity.public_key, name="typo")

    signature, timestamp = identity.deletion("typo")
    await funduq.delete_agent(identity.public_key, "typo", signature, timestamp)
...
async def test_deleting_an_agent_that_never_existed_is_not_found(funduq):
    identity = _Identity()
    await identity.register(funduq, "real")
...
async def test_an_agent_someone_is_serving_is_refused(funduq, attach):
    identity = _Identity()
    await identity.register(funduq, "local")
    await attach(identity, _Provider(), ["local"])

    signature, timestamp = identity.deletion("local")
    with pytest.raises(AgentInUse) as refused:
        await funduq.delete_agent(identity.public_key, "local", signature, timestamp)
    assert refused.value.reason == "connected"


async def test_an_agent_with_a_paused_run_is_refused(funduq):
    identity = _Identity()
    registered = await identity.register(funduq, "paused")
    ...
    with pytest.raises(AgentInUse) as refused:
        await funduq.delete_agent(identity.public_key, "paused", signature, timestamp)
    assert refused.value.reason == "active_run"


async def test_an_agent_that_has_held_a_conversation_is_refused(funduq, attach):
    identity = _Identity()
    registered = (await identity.register(funduq, "worked")).agents
    await attach(identity, _Provider(), ["worked"])

    handle = await funduq.start_run(registered["worked"], {"messages": []})
    ...
    with pytest.raises(AgentInUse) as refused:
        await funduq.delete_agent(identity.public_key, "worked", signature, timestamp)
    assert refused.value.reason == "has_history"

    await identity.register(funduq, "something-else")
    assert [a.online for a in await funduq.list_agents() if a.name == "worked"] == [False]
    assert await funduq.get_agent(registered["worked"]) is not None
