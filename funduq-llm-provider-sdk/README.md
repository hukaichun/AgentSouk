...
What an LLM provider and funduq agree on, stated from the provider's side: an
identity and what it signs, the link a transport implements, and the
completion envelope. The peer of `funduq-provider-sdk`, for the party that
answers KYOK completion calls with its own key.

**It carries no transport and no policy.** Dependencies are
`funduq-provider-sdk` (identity is identity) and `openai` (a completion is an
...
credential. When it needs a completion, it asks funduq; funduq relays the request
...
- `ProviderIdentity` / `sign_llm_registration` — who you are, and how you
  register offerings.
- `FunduqLLMLink` — the port a transport implements. `InProcessLLMProvider` is
...
- `DeliveredCompletion` — what your handler receives: the run's identity, the
  ...
  chain.
- `funduq_provider_sdk.verify_chain` — verify that chain yourself; no trust in
  funduq's summary needed.
...
```python
from funduq_llm_provider_sdk import CompletionRefused, DeliveredCompletion
...
you and your callers — funduq relays it and never interprets it. The same hook
