...
This is the schema souk's old db.bootstrap_schema()/SCHEMA_SQL applied
directly at server startup, transplanted verbatim into the first
migration — see souk/db.py's git history for the per-table reasoning
behind each comment below. From here on, schema changes are new
...
-- (see api_a2a._start_run). `gen_random_bytes` needs pgcrypto. Pinned to
-- `public` explicitly (independent of SOUK_DB_SCHEMA/search_path) since
-- it's a shared extension, not one of souk's own objects — env.py always
-- keeps `public` in search_path too, so it's still reachable unqualified.
