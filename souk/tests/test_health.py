...
async def test_a_reaped_pause_gets_its_terminal_event_persisted(session, souk, new_identity):
    from souk.health import close_with_terminal_event
...
async def test_a_run_that_reported_its_own_error_is_left_alone(session, souk, new_identity):
    from souk.health import close_with_terminal_event
...
async def test_orphans_reaped_at_start_get_terminal_events(settings, session, new_identity):
    ...
    try:
        ...
        assert events[-1] == {"type": "RUN_ERROR", "message": "orphaned_by_souk_restart"}
    finally:
        await reborn.aclose()
