...
// src/app.ts
var SOUK_URL_KEY = "souk-directory:soukUrl";
var DEFAULT_SOUK_URL = "http://localhost:8000";
function shortKey(publicKey) {
  return publicKey.length <= 14 ? publicKey : `${publicKey.slice(0, 8)}\u2026${publicKey.slice(-6)}`;
}
function groupByProvider(agents) {
...
function setSoukUrl(url) {
  const clean = url.trim().replace(/\/$/, "");
  localStorage.setItem(SOUK_URL_KEY, clean);
  return clean;
}
function linkWithSouk(path, soukUrl) {
  const url = new URL(path, window.location.href);
  url.searchParams.set("souk", soukUrl);
  return url.toString();
}
function renderSoukBar(containerEl, onChange) {
...
async function fetchAgents(soukUrl) {
