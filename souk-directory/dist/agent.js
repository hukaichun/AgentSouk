...
function renderMarkdown(el, raw) {
  el.innerHTML = purify.sanitize(g.parse(raw, { async: false }));
  el.classList.add("markdown");
}
var params = new URLSearchParams(window.location.search);
var agentId = params.get("id");
var threadId = null;
var selfName = "assistant";
var currentAssistantEl = null;
var currentAssistantRaw = "";
var typingEl = null;
var agentNameById = /* @__PURE__ */ new Map();
var subAgentEls = /* @__PURE__ */ new Map();
var subAgentRaw = /* @__PURE__ */ new Map();
var toolCallEls = /* @__PURE__ */ new Map();
var reasoningEls = /* @__PURE__ */ new Map();
function appendEntry(tag, text2, cssClass) {
...
function showTyping() {
  ...
  typingEl = bubble.parentElement;
}
function clearTyping() {
  typingEl?.remove();
  typingEl = null;
}
async function loadAgentInfo(soukUrl) {
  ...
  if (!agent) {
    ...
    document.getElementById("agent-desc").textContent = "It may be offline past the staleness window, or de-listed.";
  ...
  if (!agent.online) {
    document.getElementById("offline-banner").innerHTML = `<div class="offline-banner">Currently offline \u2014 your message will wait briefly for it to come back, then fail if it doesn't.</div>`;
  }
  return agent;
}
function handleAguiEvent(event) {
  ...
  if (event.type === "TOOL_CALL_START") {
    clearTyping();
    const el = appendEntry(`tool \xB7 ${event.toolCallName || "?"}`, "\u2026", "tool");
  ...
  if (event.type === "CUSTOM" && event.name === "sub_agent_progress") {
    ...
    const textParts = (value.artifact?.parts || []).filter((p) => p.type === "text" && p.text);
...
async function renderCallChain(soukUrl, forThreadId) {
...
async function sendMessage(soukUrl, text2) {
  const sendBtn = document.getElementById("chat-send");
  sendBtn.disabled = true;
  appendEntry("you", text2, "user");
  currentAssistantEl = null;
  currentAssistantRaw = "";
  subAgentEls.clear();
  subAgentRaw.clear();
  ...
  try {
    ...
    const body = {
      ...
      state: null,
      messages: [{ id: crypto.randomUUID(), role: "user", content: text2 }],
      tools: [],
      context: [],
      forwardedProps: null
    };
...
async function init(soukUrl) {
  ...
  document.getElementById("chat-form").addEventListener("submit", (e) => {
    ...
    if (!text2) return;
    input.value = "";
    sendMessage(soukUrl, text2);
  });
}
var initial = renderSoukBar(document.getElementById("souk-bar"), init);
init(initial);
